...
run_step() {
  ...
  if [[ -s "$output_path" ]]; then
    echo "[skip] $label -> existing $output_path"
    wc -l "$output_path"
    return 0
  fi
  echo "[build] $label"
  ...
  if [[ $status -ne 0 ]]; then
    if [[ -s "$output_path" ]]; then
      echo "[warn] $label exited with status $status but output exists; continuing"
    else
      echo "[error] $label failed and no output was produced" >&2
      return $status
    fi
  fi
  wc -l "$output_path"
...
run_step "plain dialogue half" "$BUILD_ROOT/plain_dialogue_half.jsonl" \
...
run_step "pure code half" "$BUILD_ROOT/pure_code_half.jsonl" \
...
run_step "pure math half" "$BUILD_ROOT/pure_math_half.jsonl" \
  "$PYTHON_EXE" scripts/build_segmented_dialogue_corpus.py \
...
run_step "pure wiki half" "$BUILD_ROOT/pure_wiki_half.jsonl" \
  "$PYTHON_EXE" scripts/build_segmented_dialogue_corpus.py \
...
run_step "pure pubmed half" "$BUILD_ROOT/pure_pubmed_half.jsonl" \
  "$PYTHON_EXE" scripts/build_segmented_dialogue_corpus.py \
...
run_step "pure arxiv all" "$BUILD_ROOT/pure_arxiv_all.jsonl" \
  "$PYTHON_EXE" scripts/build_segmented_dialogue_corpus.py \
...
run_step "mixed dialogue all" "$BUILD_ROOT/mixed_dialogue_all.jsonl" \
  "$PYTHON_EXE" scripts/build_segmented_dialogue_corpus.py \
...
run_step "math qa all" "$BUILD_ROOT/math_qa_all.jsonl" \
  "$PYTHON_EXE" scripts/build_math_qa_corpus.py \
  --output "$BUILD_ROOT/math_qa_all.jsonl" \
  --max-records-per-source 1000000 \
  --skip-failed-sources

run_step "reasoning all" "$BUILD_ROOT/reasoning_all.jsonl" \
