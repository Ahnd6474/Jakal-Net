...
class ArchitecturalModuleTests(unittest.TestCase):
    ...
    def test_s_module_can_linearly_ramp_residual_gate_init_by_layer(self) -> None:
        s_module = SModule(
            vocab_size=32,
            dim=8,
            max_seq_len=12,
            ...
            implementation="reference",
            s_window=12,
            propagation_residual_gate_init=0.1,
            propagation_residual_gate_init_max=1.0,
        )

        actual = [float(block.residual_gate.detach().item()) for block in s_module.sequence_stack.blocks]
